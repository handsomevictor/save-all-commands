...
> **为什么"只填入，不执行"很重要？**
> 绝大多数命令管理工具（包括许多 fzf 脚本）选中后会立即执行命令。`sac` 的 shell integration 经过专门设计：TUI 渲染走 stderr，选中结果走 stdout，shell 函数捕获后写入 `BUFFER` 而非 `eval`。这意味着带 `{namespace}`、`{pod}` 等占位符的命令，你有机会先改参数，再决定是否执行，不会误跑任何东西。
...
>
> 另外，大多数 fzf 脚本选中后会直接 `eval` 结果——`sac` 不会。TUI 渲染走 stderr，命令输出走 stdout，shell 函数只做 `BUFFER=$result`，**零执行风险**。
